...
$sql3 = $con->prepare(
  "SELECT id FROM ownerprofile WHERE username = ?");
...
if(!empty($userDouble)){
	...
	$sql4 = $con->prepare("UPDATE ownerprofile SET pets = ?, smoker = ?, age = ?,  
	...
	//New user profile
	$sql2 = "INSERT INTO ownerprofile (id, username, pets, smoker, age, education, work, income)
	VALUES ('".$userID."', '".$_SESSION['user']."', '".$pets."','".$smoker."','".$age."','".$schooling."','".$work."','".$income."')";

	if ($con->query($sql2) === FALSE) 
		echo "Error: " . $sql2 . "<br>" . $con->error;
}
header('Location: ownerTenantSelectionPage.php');
?>